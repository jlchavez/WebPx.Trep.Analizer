...
namespace WebPx.Trep.Exporter
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                if (render)
                {
                    ...
                    Console.WriteLine("[D] - Descargar actas y fechas de Simple Proof");
                    Console.WriteLine();
        ...
        private static void DescargarActas()
        {
            ScreenHeader();
            ...
            if (elecciones?.Length > 0)
            {
                do
                {
                    ...
                    {
                        ScreenHeader();
                        Console.WriteLine("Extraer fecha y hora de publicación de actas");
                        Console.WriteLine();
                        Console.WriteLine($"Elija que actas quiere descargar ({fuente.Humanize()})");
                        Console.WriteLine();
                        int i = 1;
                    ...
                    switch (key.Key)
                    {
                        case ConsoleKey.C: return;
                        case ConsoleKey.T:
                            (target, process) = (elecciones, true);
                            break;
                        default:
                    ...
                    if (process)
                    {
                        ...
                        var targetFilename = $"{fuente}_{date}_mapa.xlsx";
                        var task = utils!.DescargarActas(target, targetFilename);
                        task.Wait();
        ...
        private static void ExtraerFechaHora()
        {
            ...
            if (elecciones?.Length > 0)
            {
                do
                {
                    ...
                    {
                        ScreenHeader();
                        Console.WriteLine("Descargar actas y fechas de Simple Proof");
                        Console.WriteLine();
                        Console.WriteLine($"Elija que actas quiere procesar ({fuente.Humanize()})");
                        Console.WriteLine();
                        int i = 1;
                        foreach (var eleccion in elecciones)
                        {
